...
func (i InlineQueryResultsButton) Validate() error {
	...
	if i.WebApp != nil {
		if er := i.WebApp.Validate(); er != nil {
			err = append(err, er)
		}
	}
...
func (i InlineQueryResultPhoto) Validate() error {
	...
	if i.ParseMode != nil && i.CaptionEntities != nil {
		err = append(err, fmt.Errorf("parse_mode can't be enabled if caption_entities are provided"))
	}
	if i.ReplyMarkup != nil {
		if er := i.ReplyMarkup.Validate(); er != nil {
			err = append(err, er)
		}
	}
...
func (i InlineQueryResultGif) Validate() error {
	...
	if i.ParseMode != nil && i.CaptionEntities != nil {
		err = append(err, fmt.Errorf("parse_mode can't be used if caption_entities are provided"))
	}
	if i.InputMessageContent != nil {
		if er := i.InputMessageContent.Validate(); er != nil {
			err = append(err, er)
...
func (i InlineQueryResultVideo) Validate() error {
	...
	if i.ParseMode != nil && i.CaptionEntities != nil {
		err = append(err, fmt.Errorf("parse mode can't be enabled if Entities are provided"))
	}
	if i.ReplyMarkup != nil {
		if er := i.ReplyMarkup.Validate(); er != nil {
			err = append(err, er)
...
func (i InlineQueryResultCachedMpeg4Gif) Validate() error {
	...
	if i.Mpeg4FileId == "" {
		err = append(err, fmt.Errorf("mpeg_4_file_id parameter can't be empty"))
	}
	if i.ParseMode != nil && i.CaptionEntities != nil {
		err = append(err, fmt.Errorf("parse_mode can't be used if caption_entities are provided"))
	}
...
func (i InlineQueryResultCachedSticker) Validate() error {
	...
	if i.StickerFileId == "" {
		err = append(err, fmt.Errorf("sticker_file_id parameter can't be empty"))
	}
	if i.ReplyMarkup != nil {
		if er := i.ReplyMarkup.Validate(); er != nil {
			err = append(err, er)
...
func (i InlineQueryResultCachedDocument) Validate() error {
	...
	if i.DocumentFileId == "" {
		err = append(err, fmt.Errorf("document_file_id parameter can't be empty"))
	}
	if i.ParseMode != nil && i.CaptionEntities != nil {
		err = append(err, fmt.Errorf("parse_mode can't be used if caption_entities are provided"))
	}
...
func (i InputTextMessageContent) Validate() error {
	...
	if l := len(i.MessageText); l < 1 || l > 4096 {
		err = append(err, fmt.Errorf("message_text parameter must be between 1 and 4096 characters"))
	}
	...
	if i.LinkPreviewOptions != nil {
		if er := i.LinkPreviewOptions.Validate(); er != nil {
			err = append(err, er)
		}
	}
	if len(err) > 0 {
...
func (i InputLocationMessageContent) Validate() error {
	...
	if i.LivePeriod != nil {
		if (*i.LivePeriod < 60 || *i.LivePeriod > 86400) && *i.LivePeriod != 0x7FFFFFFF {
			err = append(err, fmt.Errorf("live_period parameter must be between 60 and 86400 or equal to 0x7FFFFFFF"))
		}
	}
	...
	if i.Heading != nil {
		if *i.Heading < 1 || *i.Heading > 360 {
			err = append(err, fmt.Errorf("heading parameter must be between 0 and 1500"))
		}
	}
	if i.ProximityAlertRadius != nil {
		if *i.ProximityAlertRadius < 1 || *i.ProximityAlertRadius > 100000 {
			err = append(err, fmt.Errorf("proximity_alert_radius must be between 1 and 100000 meters"))
		}
	}
	if len(err) > 0 {
		return err
...
func (i InputInvoiceMessageContent) Validate() error {
	...
	if l := len(i.Title); l < 1 || l > 32 {
		err = append(err, fmt.Errorf("title parameter must be between 1 and 32 characters"))
	}
	if l := len(i.Description); l < 1 || l > 255 {
		err = append(err, fmt.Errorf("description parameter must be between 1 and 255 characters"))
	}
	if l := len([]byte(i.Payload)); l < 1 || l > 128 {
		err = append(err, fmt.Errorf("payload parameter must be between 1 and 128 bytes"))
	}
	if len(i.Currency) > 3 {
		err = append(err, fmt.Errorf("currency parameter accepts only valid three-letter ISO 4217 currency codes"))
	}
	if i.Currency == "XTR" {
		if len(i.Prices) > 1 {
			err = append(err, fmt.Errorf("prices parameter must contain exactly one item for payments in Telegram Stars."))
		}
		if i.MaxTipAmount != nil {
			err = append(err, fmt.Errorf("max_tip_amount parameter is not supported for payments in Telegram stars"))
		}
	}
	if i.SuggestedTipAmounts != nil {
		if len(*i.SuggestedTipAmounts) > 4 {
			err = append(err, fmt.Errorf("suggested_tip_amounts parameter can't be longer than 4 elements"))
		}
		for j := 0; j < len(*i.SuggestedTipAmounts); j++ {
			if (*i.SuggestedTipAmounts)[j] <= 0 {
				err = append(err, fmt.Errorf("suggested_tip_amounts parameter accepts only positive integers"))
			}
			if (*i.SuggestedTipAmounts)[j] > *i.MaxTipAmount {
				err = append(err, fmt.Errorf("suggested_tip_amounts parameter accepts only integers that do not exceed the max_tip_amount"))
			}
			if j > 0 && (*i.SuggestedTipAmounts)[j] <= (*i.SuggestedTipAmounts)[j-1] {
				err = append(err, fmt.Errorf("suggested_tip_amounts parameter should be passed in a strictly increased order"))
			}
		}
	}
	if len(err) > 0 {
